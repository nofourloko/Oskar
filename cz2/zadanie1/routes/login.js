...
const { isAuthenticated, isUserInDb, hashPassword, addNewUserToDb, comparePasswords } = require('../controller/loginController')
const USER = require('../Model/credentials')
const router = express.Router()

router.use("/", async (req, res, next ) => {
    const userIn = isAuthenticated(req)
    if(userIn){
        const user = await isUserInDb(userIn)
        return res.redirect(`/departaments/${user.dept}`);
    }else{
        next()
    }
})
